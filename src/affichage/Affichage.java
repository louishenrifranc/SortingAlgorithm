...
public class Affichage extends JFrame {
	...
	private static final long serialVersionUID = 1L;
											private static Vector<Integer> nombre_valeurs_statistique;
												private ArrayList<TriPanel> listeTri; // Liste contenant les TriPanel
													// selectionnees
	private int[] index_des_Tri_Selectionnes; // Liste des index des tris
	// choisis
	private int[] index_des_generation_de_valeurs; // Liste des index des nombre
	// a
													// générer
	...
	public Affichage(int[] index_tri_choisi, int[] index_gene_nombre_choisi,
		...
		if (!stat_ou_visualisation)
			affichageholder.setLayout(optimizedLayout());
		...
		for (int i = 0; i < index_tri_choisi.length; i++) {
			...
			case Constante.codeHSshort:
				if (!stat_ou_visualisation)
					listeTri.add(new affichage.HeapSort(
							Constante.algorithme[index_tri_choisi[i]],
							temps_animation, width, height));
				else if (stat_ou_visualisation)
					selectionAlgo |= Constante.codeHeapSort;
				break;
			case Constante.codeBSshort:
				if (!stat_ou_visualisation)
					listeTri.add(new affichage.BubbleSort(
							Constante.algorithme[index_tri_choisi[i]],
							temps_animation, valeurMax, valeurMin));
				else if (stat_ou_visualisation)
					selectionAlgo |= Constante.codeBubbleSort;

				break;
			case Constante.codeQSshort:
				if (!stat_ou_visualisation)
					listeTri.add(new affichage.QuickSort(
							Constante.algorithme[index_tri_choisi[i]],
							temps_animation, width, height));
				else if (stat_ou_visualisation)
					selectionAlgo |= Constante.codeQuickSort;

				break;
			case Constante.codeSSshort:
				if (!stat_ou_visualisation)
					listeTri.add(new affichage.SelectionSort(
							Constante.algorithme[index_tri_choisi[i]],
							temps_animation, width, height));
				else if (stat_ou_visualisation)
					selectionAlgo |= Constante.codeSelectionSort;
		...
		if (!stat_ou_visualisation) {
			...
			for (int i = 0; i < nombre_valeurs_statistique.size(); i++) {
				nombre_valeur_visualisation = nombre_valeurs_statistique.get(i);
				genererNombre();
			...
			SwingUtilities.invokeLater(new Runnable() {
				...
				public void run() {
					...
					StatistiqueFenetre demo = new StatistiqueFenetre(nombre_valeurs,
							selectionAlgo, buffer);
	...
	public void beginAnimation() throws InterruptedException {
		...
		ExecutorService executor = Executors
				.newFixedThreadPool(listeTri.size());
		for (int i = 0; i < listeTri.size(); i++) {
	...
	private Vector<Integer> genererTendanceCroissante() {
		...
		for (int i = 0; i < nombre_valeur_visualisation; i++) {
			...
			list_valeurs_a_trier.add(((i % changement == 0) ? rand.nextInt(Math
					.abs(valeur_max_liste - valeur_min_liste))
					+ valeur_min_liste : valeur));
		}
		return list_valeurs_a_trier;
