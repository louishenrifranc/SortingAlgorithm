...
public class StatistiqueFenetre extends JFrame
{
    ...
    public Vector<Integer>          _valeurs;                                        // Stocke
	                                                                              // les
	                                                                              // tailles
	                                                                              // des
	                                                                              // differentes
    // listes
    public int                      _algos;                                          // Un
	                                                                              // integer
	                                                                              // représentant
	                                                                              // les
	                                                                              // algorithmes
	                                                                              // séléctionnés
    Vector<Vector<Integer>>         _listes;                                         // Un
	                                                                              // tableau
	                                                                              // de
	                                                                              // liste
	                                                                              // d'elements
    ...
    private BigInteger              nombreValeurDeplace;                             // Le
	                                                                              // nombre
	                                                                              // de
	                                                                              // valeurs
	                                                                              // deplaces,
	...
	public void ajouterHeapSortUtil(Integer nombre, ArrayList<Integer> _heap)
	{
		...
		if (_heap.get( currentIndex ).compareTo(
		        _heap.get( parentIndex ) ) > 0)
		{
	...
	private Tuple bubbleSort(Vector<Integer> list, int k)
	{
	    ...
	    return new Tuple( nombreValeurDeplace , new BigInteger(
		    Long.toString( tempsTri ) ) );
	}
	        ...
	        ArrayList<Integer> _heap)
	{
		...
		if (leftChildIndex >= _heap.size()) break;
	...
	public Integer gestionAlgorithmes()
	{
	    ...
	    BigInteger temps[][] = new BigInteger[nombreAlgo + 1][_valeurs
		    .size() + 2];
	    BigInteger nbresvaleursdeplaces[][] = new BigInteger[nombreAlgo + 1][_valeurs
		    .size() + 2];

	    for (int i = 1; i <= _valeurs.size(); i++)
	    {
		temps[0][i] = new BigInteger( Integer.toString( _valeurs
		        .elementAt( i - 1 ) ) );
		...
		if ( ( _algos & ( Constante.codeSelectionSort ) ) != 0)
		{
		    ...
		    temps[j][0] = new BigInteger(
			    Integer.toString( Constante.codeSSshort ) );
		    nbresvaleursdeplaces[j][0] = new BigInteger(
			    Integer.toString( Constante.codeSSshort ) );
		...
		if ( ( _algos & ( Constante.codeQuickSort ) ) != 0)
		{ // Si QuickSort
		  // a
		  // été
		  // séléctionnés
	    ...
	    /*
	     * for(int i=0 ; i<nombreAlgo+1;i++){ for(int
	     * j=0;j<=_valeurs.size();j++) {
	     * System.out.print(nbresvaleursdeplaces[i][j]+" "); }
	     * System.out.println(); }
	     * System.out.println("-------------------------"); for(int i=0 ;
	     * i<nombreAlgo+1;i++){ for(int j=0;j<=_valeurs.size();j++) {
	     * System.out.print(temps[i][j]+" "); } System.out.println(); }
	     */
	...
	private Tuple heapSort(Vector<Integer> liste, int k)
	{
	    ...
	    for (int i = 0; i < _valeurs.elementAt( k ); i++)
	    { // Ajouter un
	      // element
	      // a la
	      // pile
	    ...
	    for (int i = _valeurs.elementAt( k ) - 1; i >= 0; i--)
	    { // Supprimer
	      // les
	      // elements de la
	...
	{
	    long tempsDebut = System.nanoTime();
	    BigInteger nbrevd = quicksort( liste , 0 ,
		    _valeurs.elementAt( k ) - 1 );
	    Long tempsFin = System.nanoTime();
	    long tempsTri = ( Math.abs( tempsDebut - tempsFin ) );

	    return new Tuple( nbrevd ,
		    new BigInteger( Long.toString( tempsTri ) ) );
	}
